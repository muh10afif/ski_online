            ...
            <div class="scroll-sidebar">
                ...
                <nav class="sidebar-nav">
                    <ul id="sidebarnav" class="p-t-30">
                       <li class="sidebar-item"> <a class="sidebar-link waves-effect waves-dark sidebar-link" href="<?= base_url() ?>Atasan1/" aria-expanded="false"><i class="fas fa-home"></i><?php echo nbs(2) ?><span class="hide-menu">Dashboard</span></a></li>
                        <li class="sidebar-item"> <a class="sidebar-link waves-effect waves-dark sidebar-link" href="<?= base_url() ?>Atasan1/karyawan" aria-expanded="false"><i class="fas fa-check-square"></i><?php echo nbs(2) ?><span class="hide-menu">Approval SKI Karyawan</span></a></li><hr>
                    <?php if($this->session->userdata('ses_role')=='Admin'|| $this->session->userdata('ses_role1')=='Admin'|| $this->session->userdata('ses_role2')=='Admin'){;?>
                        <li class="sidebar-item"> <a class="sidebar-link waves-effect waves-dark sidebar-link" href="<?php echo base_url()?>admin" aria-expanded="false"><i class="fas fa-user-circle"></i><?php echo nbs(2) ?><span class="hide-menu">Halaman Akses Admin</span></a></li>
                    <?php } else{}?>
...
<script type="text/javascript">
    ...
    $("input").keyup( function() {
      ...
      if (!isNaN(tol_bot)) 
        ...
        function tampil_ubah_penetapan(){
         var url;
          ...
          if (a < 100 ) {
              ...
              $('.modal-title').text('Total Bobot Kurang dari 100%'); 
               ...
               url = "<?php echo site_url('Atasan1/ubah_nilai')?>";
         ...
         $.ajax({
            ...
            {
                $('#save').hide();
                  url = "<?php echo site_url()?>Atasan1/karyawan_penetapan/<?php echo $param_nipeg ?>";
        ...
        function tampil_ubah_penilaian(){
         var url;
 
               url = "<?php echo site_url('Atasan1/simpan_nilai_real')?>";        
         ...
         $.ajax({
            ...
            {
                $('#ubah').hide();
                url = "<?php echo site_url()?>Atasan1/karyawan_penilaian/<?php echo $param_nipeg ?>";
         ...
         if(confirm('Apakah yakin akan mengirim Data Penetapan SKI ?'))
            {
                ...
                $.ajax({
                    url : "<?php echo site_url('Atasan1/submit_nilai')?>/"+id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data)
                    {
        ...
        function submit_nilai(id){
         if(confirm('Apakah yakin akan mengirim Data Penilaian SKI ?'))
            {
                // ajax delete data to database
                $.ajax({
                    url : "<?php echo site_url('Atasan1/submit_nilai_real')?>/"+id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data)
                    {
                        $('#ubah').hide();
                        $('#approve').hide();
                       url = "<?php echo site_url()?>ski_mail/index/<?php echo $param_nipeg.'/'.$this->session->userdata('ses_nipeg');?>";
...
<script type="text/javascript">
    ...
    ***********************************************************/
    ...
    var x = setInterval(function() { 
        ...
        if (distance < 0) {
            clearInterval(x);
            document.getElementById("tutup").innerHTML = "<div class='alert alert-warning' style='margin-top:30px;' role='alert'><h3 style='text-align: center; '><?php echo $status; ?> TELAH DITUTUP!</h3></div>";
        } 
